...
void Map::resetMap()
{
    ...
    for (int i = 0; i < max_tables_; i++)
    {
        table_positions_[i][0] = -1;
        table_positions_[i][1] = -1;
    }
    table_count_ = 0;
    ...
    map_data_[2][2] = LUNCH_BREAK; // placement of the lunch break
    map_data_[2][9] = KITCHEN; // placememnt of the kitchen
}
...
bool Map::isCriticLocation(int row, int col)
{
    if (!isOnMap(row, col))
    {
        return false;
    }
    for (int i = 0; i < critic_count_; i++)
    {
        if (row == critic_positions_[i][0] && col == critic_positions_[i][1])
        {
            return true;
        }
    }
 ...
 * Return: boolean (bool)
 */
bool Map::isTableLocation(int row, int col)
{
    if (!isOnMap(row, col))
    {
        return false;
    ...
    {
        if (row == table_positions_[i][0] && col == table_positions_[i][1])
        {
            return true;
        }
    }
...
bool Map::addNPC(int row, int col)
{
    ...
    npc_positions_[npc_count_][2] = true; // *** changes visibility ***
 ...
 * if (row,col) is not a free space
 *      return false
 * if next row in table_positions_ matrix is -1 -1
 *      store row, col and type values in the current row of table_positions_ matrix
 ...
 * Return: boolean (bool)
 */
bool Map::addTable(int row, int col)
{

    if (table_count_ >= max_tables_)
    {
        return false;
    }
    ...
    table_positions_[table_count_][0] = row;
    table_positions_[table_count_][1] = col;
    table_count_++;
    if(row == 5 && col == 2){
        map_data_[row][col] = TABLE1;
    }
    else if(row == 6 && col == 9){
        map_data_[row][col] = TABLE2;
    }
    else if(row == 7 && col == 2){
        map_data_[row][col] = TABLE3;
    }
    else if(row == 8 && col == 9){
        map_data_[row][col] = TABLE4;
    }
    else if(row == 9 && col == 2){
        map_data_[row][col] = TABLE5;
    }
    return true;
}

bool Map::addCritic(int row, int col)
{
    if (critic_count_ >= max_critic_)
    {
        return false;
    }
    if (!isFreeSpace(row, col))
    ...
    critic_positions_[critic_count_][0] = row;
    critic_positions_[critic_count_][1] = col;
    critic_positions_[critic_count_][2] = true; // *** changes visibility ***
    map_data_[row][col] = CRITIC; 
    critic_count_++;
    return true;
}
 ...
 * Return: boolean (bool)
 */
bool Map::removeTable(int row, int col)
{
    for (int i = 0; i < table_count_; i++)
    {
        if (table_positions_[i][0] == row && table_positions_[i][1] == col)
        {
            // swap i'th table with last table
            table_positions_[i][0] = table_positions_[table_count_ - 1][0];
            table_positions_[i][1] = table_positions_[table_count_ - 1][1];
            // reset last table
            table_positions_[table_count_ - 1][0] = -1;
            table_positions_[table_count_ - 1][1] = -1;
            // decrement table_count_
            table_count_--;
            // set map data to explored
            map_data_[row][col] = EXPLORED;
            return true;
 ...
 bool Map::removeCritic(int row, int col)
 {
    for (int i = 0; i < critic_count_; i++)
    {
        if (critic_positions_[i][0] == row && critic_positions_[i][1] == col)
        {
            // swap i'th npc with last npc
            critic_positions_[i][0] = critic_positions_[critic_count_ - 1][0];
            critic_positions_[i][1] = critic_positions_[critic_count_ - 1][1];
            critic_positions_[i][2] = critic_positions_[critic_count_ - 1][2];
            // reset last npc
            critic_positions_[npc_count_ - 1][0] = -1;
            critic_positions_[npc_count_ - 1][1] = -1;
            critic_positions_[npc_count_ - 1][2] = false;
            // decrement npc_count_
            critic_count_--;
            // set map data to explored
            map_data_[row][col] = EXPLORED;
